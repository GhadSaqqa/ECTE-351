__name__ = 'Web_BeeHive_Indoor'

# ============================================================
# INDOOR BEEHIVE HUB / DASHBOARD
# ============================================================
#
# PURPOSE:
# This is the INDOOR receiver/dashboard code.
#
# At the moment, the Kookaberry can run by itself.
# No sensors or HC-12 need to be connected yet.
#
# FUTURE SYSTEM:
#
# OUTDOOR BEEHIVE SYSTEM
#     |
#     | Sensors
#     |
# Outdoor Pico
#     |
#     | HC-12 radio
#     v
# Indoor HC-12
#     |
#     v
# Indoor Kookaberry
#     |
#     v
# Wi-Fi Dashboard
#
# IMPORTANT:
# The Kookaberry's Wi-Fi does NOT require internet.
# It creates its own Wi-Fi network.
#
# ============================================================


# ============================================================
# IMPORT NECESSARY MODULES
# ============================================================
#
# network -> creates the Kookaberry Wi-Fi network
# socket  -> creates the webpage server
# time    -> timing functions
# kooka   -> Kookaberry hardware/display
# fonts   -> Kookaberry display fonts
#
# ============================================================

import network
import socket
import time

import kooka
import fonts


# ============================================================
# WI-FI SETTINGS
# ============================================================
#
# The Kookaberry creates its own Wi-Fi network.
#
# INTERNET IS NOT REQUIRED.
#
# Connect your laptop/phone to this network and enter the
# IP address shown on the Kookaberry into a web browser.
#
# ============================================================

ssid = 'kookaberry'
password = '12345678'

disp = kooka.display


# ============================================================
# HC-12 RECEIVER
# ============================================================
#
# FUTURE:
# The indoor HC-12 will receive the sensor data transmitted
# by the outdoor HC-12.
#
# The exact UART and pin numbers depend on the final wiring.
#
# CHANGE ACCORDING TO PIN ALLOCATION
#
# ============================================================

# from machine import Pin, UART

# HC12_TX_PIN = 'CHANGE ACCORDING TO PIN ALLOCATION'
# HC12_RX_PIN = 'CHANGE ACCORDING TO PIN ALLOCATION'

# hc12 = UART(
#     1,
#     baudrate=9600,
#     tx=Pin(HC12_TX_PIN),
#     rx=Pin(HC12_RX_PIN)
# )


# ============================================================
# SENSOR DATA
# ============================================================
#
# These variables will eventually contain the values received
# from the outdoor system through the HC-12.
#
# They are currently set to 0 so the code can run without
# any sensors or radio connected.
#
# ============================================================


# ------------------------------------------------------------
# TEMPERATURE
# ------------------------------------------------------------

# Outdoor Temperature
# SENSOR: BME280
# CHANGE SENSOR IF FINAL WIRING USES A DIFFERENT SENSOR

outdoor_temperature = 0


# Beehive Temperature
# SENSOR: DHT22
# CHANGE SENSOR IF FINAL WIRING USES A DIFFERENT SENSOR

beehive_temperature = 0


# ------------------------------------------------------------
# HUMIDITY
# ------------------------------------------------------------

# Humidity
# SENSOR: DHT22
# CHANGE SENSOR IF FINAL WIRING USES A DIFFERENT SENSOR

humidity = 0


# ------------------------------------------------------------
# AIR PRESSURE
# ------------------------------------------------------------

# Air Pressure
# SENSOR: BME280

air_pressure = 0


# ------------------------------------------------------------
# LIGHT
# ------------------------------------------------------------

# Light Level
# SENSOR: VEML7700

light_level = 0


# ------------------------------------------------------------
# GPS / LOCATION
# ------------------------------------------------------------

# Location
# SENSOR: NEO-6M GPS

gps_latitude = 0
gps_longitude = 0
gps_altitude = 0


# GPS date and time
# SENSOR: NEO-6M GPS

recorded_date = '--/--/----'
recorded_time = '--:--:--'


# ------------------------------------------------------------
# HIVE SOUND
# ------------------------------------------------------------

# Hive Sound
# SENSOR: Microphone

hive_sound = 0


# ------------------------------------------------------------
# POWER
# ------------------------------------------------------------

# Solar Voltage
# SENSOR: Solar Cell Monitoring Module

solar_voltage = 0


# Solar Current
# SENSOR: Solar Cell Monitoring Module

solar_current = 0


# ============================================================
# WEBPAGE FUNCTION
# ============================================================
#
# This function creates the dashboard webpage.
#
# The browser receives this HTML from the Kookaberry.
#
# ============================================================

def webpage(
    outdoor_temperature,
    beehive_temperature,
    humidity,
    air_pressure,
    light_level,
    gps_latitude,
    gps_longitude,
    gps_altitude,
    solar_voltage,
    solar_current,
    hive_sound,
    recorded_date,
    recorded_time
):

    html = f"""
    <!DOCTYPE html>

    <html>

    <head>

        <title>BeeHive Monitoring System</title>

        <meta name="viewport"
        content="width=device-width, initial-scale=1">

        <style>

            body {{
                min-width: 310px;
                max-width: 800px;
                margin: 0 auto;
                font-family: Arial;
                padding: 10px;
            }}

            h1 {{
                text-align: center;
                font-size: 2.3rem;
            }}

            h2 {{
                text-align: center;
                font-size: 1.8rem;
                margin-top: 30px;
            }}

            p {{
                font-size: 1.3rem;
                text-align: center;
            }}

            .recorded {{
                font-size: 1.1rem;
            }}

        </style>

    </head>


    <body>


        <h1>BeeHive Monitoring System</h1>


        <!-- =================================================
             BEEHIVE PROPERTIES
             ================================================= -->

        <h2>Properties</h2>


        <p>
            Outdoor Temperature:
            {outdoor_temperature} C
        </p>


        <p>
            Beehive Temperature:
            {beehive_temperature} C
        </p>


        <p>
            Humidity:
            {humidity} %
        </p>


        <p>
            Air Pressure:
            {air_pressure} hPa
        </p>


        <p>
            Light Level:
            {light_level} lux
        </p>


        <!-- =================================================
             LOCATION / GPS
             ================================================= -->

        <h2>Location / GPS</h2>


        <p>
            Latitude:
            {gps_latitude}
        </p>


        <p>
            Longitude:
            {gps_longitude}
        </p>


        <p>
            Altitude:
            {gps_altitude} m
        </p>


        <!-- =================================================
             POWER
             ================================================= -->

        <h2>Power</h2>


        <p>
            Solar Voltage:
            {solar_voltage} V
        </p>


        <p>
            Solar Current:
            {solar_current} mA
        </p>


        <!-- =================================================
             HIVE SOUND
             ================================================= -->

        <h2>Hive Sound</h2>


        <p>
            Sound Level:
            {hive_sound}
        </p>


        <!-- =================================================
             RECORDED TIME
             ================================================= -->

        <h2>Recorded</h2>


        <p class="recorded">
            Date: {recorded_date}
        </p>


        <p class="recorded">
            Time: {recorded_time}
        </p>


        <!-- =================================================
             REFRESH
             =================================================
             
             10000 milliseconds = 10 seconds.
             
             CHANGE THIS VALUE IF NEEDED.
             
             This refreshes the webpage every 10 seconds.
             
             ================================================= -->

        <script>

        setInterval(function() {{

            location.reload(true);

        }}, 10000);

        </script>


    </body>

    </html>
    """

    return str(html)


# ============================================================
# START WI-FI ACCESS POINT
# ============================================================
#
# The Kookaberry creates a Wi-Fi network.
#
# Internet is NOT required.
#
# ============================================================

disp.print('BeeHive Dashboard')

disp.print('SSID %s' % ssid)


wlan = network.WLAN(
    network.AP_IF
)


wlan.config(
    essid=ssid,
    password=password
)


wlan.active(True)


# ============================================================
# WAIT FOR WI-FI TO ACTIVATE
# ============================================================

while wlan.active() == False:

    pass


print(
    'AP Mode Is Active, You can Now Connect'
)


# ============================================================
# GET IP ADDRESS
# ============================================================
#
# This is the IP address you enter into the browser.
#
# ============================================================

addr = wlan.ifconfig()[0]


print(
    'IP Address to connect to: ' + addr
)


disp.print('AP Active')

disp.print('Connect to:')

disp.print(addr)


# ============================================================
# CREATE WEB SERVER
# ============================================================
#
# The socket waits for a laptop/phone to request the
# dashboard webpage.
#
# ============================================================

s = socket.socket(
    socket.AF_INET,
    socket.SOCK_STREAM
)


s.bind(
    ('', 80)
)


s.listen(1)


print(
    'Listening on',
    addr
)


disp.print('Listening')


# ============================================================
# MAIN LOOP
# ============================================================

while True:


    # ========================================================
    # HC-12 RECEIVE DATA
    # ========================================================
    #
    # FUTURE:
    #
    # The outdoor system will transmit the sensor data through
    # the HC-12.
    #
    # Example data format:
    #
    # TEMP_OUT=25.4,
    # TEMP_HIVE=34.1,
    # HUM=61.2,
    # PRESS=1013.4,
    # LIGHT=450,
    # LAT=-34.4068,
    # LON=150.8821,
    # ALT=35,
    # SOLAR_V=5.2,
    # SOLAR_I=120,
    # SOUND=45,
    # DATE=12/08/2026,
    # TIME=15:42:18
    #
    # CHANGE ACCORDING TO FINAL HC-12 DATA FORMAT
    #
    # ========================================================


    # if hc12.any():

    #     data = hc12.readline()

    #     if data:

    #         data = data.decode().strip()

    #         values = data.split(',')


    #         for value in values:


    #             # ------------------------------------------
    #             # OUTDOOR TEMPERATURE
    #             # SENSOR: BME280
    #             # ------------------------------------------

    #             if value.startswith('TEMP_OUT='):

    #                 outdoor_temperature = float(
    #                     value.replace('TEMP_OUT=', '')
    #                 )


    #             # ------------------------------------------
    #             # BEEHIVE TEMPERATURE
    #             # SENSOR: DHT22
    #             # ------------------------------------------

    #             elif value.startswith('TEMP_HIVE='):

    #                 beehive_temperature = float(
    #                     value.replace('TEMP_HIVE=', '')
    #                 )


    #             # ------------------------------------------
    #             # HUMIDITY
    #             # SENSOR: DHT22
    #             # ------------------------------------------

    #             elif value.startswith('HUM='):

    #                 humidity = float(
    #                     value.replace('HUM=', '')
    #                 )


    #             # ------------------------------------------
    #             # AIR PRESSURE
    #             # SENSOR: BME280
    #             # ------------------------------------------

    #             elif value.startswith('PRESS='):

    #                 air_pressure = float(
    #                     value.replace('PRESS=', '')
    #                 )


    #             # ------------------------------------------
    #             # LIGHT
    #             # SENSOR: VEML7700
    #             # ------------------------------------------

    #             elif value.startswith('LIGHT='):

    #                 light_level = float(
    #                     value.replace('LIGHT=', '')
    #                 )


    #             # ------------------------------------------
    #             # GPS LATITUDE
    #             # SENSOR: NEO-6M GPS
    #             # ------------------------------------------

    #             elif value.startswith('LAT='):

    #                 gps_latitude = float(
    #                     value.replace('LAT=', '')
    #                 )


    #             # ------------------------------------------
    #             # GPS LONGITUDE
    #             # SENSOR: NEO-6M GPS
    #             # ------------------------------------------

    #             elif value.startswith('LON='):

    #                 gps_longitude = float(
    #                     value.replace('LON=', '')
    #                 )


    #             # ------------------------------------------
    #             # GPS ALTITUDE
    #             # SENSOR: NEO-6M GPS
    #             # ------------------------------------------

    #             elif value.startswith('ALT='):

    #                 gps_altitude = float(
    #                     value.replace('ALT=', '')
    #                 )


    #             # ------------------------------------------
    #             # SOLAR VOLTAGE
    #             # SENSOR: SOLAR MONITORING MODULE
    #             # ------------------------------------------

    #             elif value.startswith('SOLAR_V='):

    #                 solar_voltage = float(
    #                     value.replace('SOLAR_V=', '')
    #                 )


    #             # ------------------------------------------
    #             # SOLAR CURRENT
    #             # SENSOR: SOLAR MONITORING MODULE
    #             # ------------------------------------------

    #             elif value.startswith('SOLAR_I='):

    #                 solar_current = float(
    #                     value.replace('SOLAR_I=', '')
    #                 )


    #             # ------------------------------------------
    #             # HIVE SOUND
    #             # SENSOR: MICROPHONE
    #             # ------------------------------------------

    #             elif value.startswith('SOUND='):

    #                 hive_sound = float(
    #                     value.replace('SOUND=', '')
    #                 )


    #             # ------------------------------------------
    #             # GPS DATE
    #             # SENSOR: NEO-6M GPS
    #             # ------------------------------------------

    #             elif value.startswith('DATE='):

    #                 recorded_date = value.replace(
    #                     'DATE=',
    #                     ''
    #                 )


    #             # ------------------------------------------
    #             # GPS TIME
    #             # SENSOR: NEO-6M GPS
    #             # ------------------------------------------

    #             elif value.startswith('TIME='):

    #                 recorded_time = value.replace(
    #                     'TIME=',
    #                     ''
    #                 )


    #         print('Received data:')
    #         print(data)


    # ========================================================
    # UPDATE KOOKABERRY DISPLAY
    # ========================================================
    #
    # Shows the main received measurements and the IP address
    # on the physical Kookaberry display.
    #
    # ========================================================

    disp.fill(0)


    disp.setfont(fonts.mono8x8)


    disp.text(
        'BeeHive Dashboard',
        0,
        6
    )


    disp.setfont(fonts.mono6x7)


    disp.text(
        'Out: %5.1f C' % outdoor_temperature,
        0,
        20
    )


    disp.text(
        'Hive: %4.1f C' % beehive_temperature,
        0,
        30
    )


    disp.text(
        'Hum: %5.1f %%' % humidity,
        0,
        40
    )


    disp.text(
        'Press:%5.1f' % air_pressure,
        0,
        50
    )


    disp.text(
        'IP: %s' % addr,
        0,
        60
    )


    disp.show()


    # ========================================================
    # PROCESS WEB CONNECTION
    # ========================================================
    #
    # Wait for a laptop/phone to request the dashboard.
    #
    # ========================================================

    try:

        conn, addrx = s.accept()


        print(
            'Got a connection from',
            addrx
        )


        # Receive browser request

        request = conn.recv(1024)


        request = str(request)


        print(
            'Request content = %s'
            % request
        )


        # ====================================================
        # CREATE WEBPAGE
        # ====================================================

        response = webpage(

            round(
                outdoor_temperature,
                1
            ),

            round(
                beehive_temperature,
                1
            ),

            round(
                humidity,
                1
            ),

            round(
                air_pressure,
                1
            ),

            round(
                light_level,
                1
            ),

            gps_latitude,

            gps_longitude,

            gps_altitude,

            round(
                solar_voltage,
                2
            ),

            round(
                solar_current,
                1
            ),

            hive_sound,

            recorded_date,

            recorded_time

        )


        # ====================================================
        # SEND WEBPAGE
        # ====================================================

        conn.send(
            'HTTP/1.0 200 OK\r\n'
            'Content-type: text/html\r\n'
            '\r\n'
        )


        conn.send(response)


        # ====================================================
        # CLOSE CONNECTION
        # ====================================================

        conn.close()


    except OSError as e:

        try:

            conn.close()

        except:

            pass
